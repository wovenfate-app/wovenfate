 ...
 *
 * Exported (rather than kept private to useNarration) so any narration
 * backend — the browser speechSynthesis engine, or a pre-rendered
 * audio-file engine — builds the exact same queue in the exact same
 * order. A pre-rendered manifest is generated by walking this same
 * function (see scripts/generate-narration.mjs), so its entries always
 * line up index-for-index with what a narration hook queues at runtime.
 */
export function buildSpeechQueue(node) {
